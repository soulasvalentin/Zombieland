...
private:
    ...
    enum STORE_STATE{ARMAS,MEJORAS,MUNICIONES,VOLVER};
    int state;
...
void CStore::open_Store(CInventory &inventory_aux){
    ...
    switch(selec){
        ...
        case 3: changeState(VOLVER);        break;
    }
...
void CStore::render_Store(CInventory &inventory_aux){
    ...
    switch(selec){
        ...
        case MEJORAS: render_mejoras(inventory_aux); break;
        case MUNICIONES: render_municiones(inventory_aux); break;
    }
    ...
    else al_draw_text(keepcalm24,al_map_rgb(180,180,180),(ANCHO*0.34)+camx-10,(ALTO*0.15)+camy+154,ALLEGRO_ALIGN_RIGHT,"VOLVER");


    show_state();
    if(GameState.getDebug_info()) GameState.show_state_debug();

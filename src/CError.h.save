...
class CError{
    ...
    public:
        ...
        CError();
        void open_error(char *_short_text, char*_long_text, double _duration);
        void update_error();
        void render_error();
};
...
void CError::open_error(char *_short_text, char*_long_text, double _duration = 3){

    al_play_sample(audios[5],.5,0,1,ALLEGRO_PLAYMODE_ONCE, NULL);
    strcpy(short_text,_short_text);
    strcpy(long_text,_long_text);

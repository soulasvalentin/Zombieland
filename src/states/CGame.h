...
void CGame::update_game(){
    ...
    //-------------------------------
        ...
        switch(ev.keyboard.keycode){
            ...
            case ALLEGRO_KEY_1:     if(GameState.getDev_mode())player.teleport();      break;
            case ALLEGRO_KEY_2:     if(GameState.getDev_mode())Inventory.add_credits(1000); break;
            case ALLEGRO_KEY_3:     if(GameState.getDev_mode())player.hit_recived(20); break;
            case ALLEGRO_KEY_4:     scale -= 0.1;

                                break;
            // AGREGAR LOOT
            case ALLEGRO_KEY_PAD_1:     if(GameState.getDev_mode()) add_loot(AMMO_PISTOL,vLoot); break;
            case ALLEGRO_KEY_PAD_2:     if(GameState.getDev_mode()) add_loot(AMMO_SHOTGUN,vLoot); break;
            case ALLEGRO_KEY_PAD_3:     if(GameState.getDev_mode()) add_loot(AMMO_UZI,vLoot); break;
            case ALLEGRO_KEY_PAD_4:     if(GameState.getDev_mode()) add_loot(AMMO_ASSAULT,vLoot); break;
            case ALLEGRO_KEY_PAD_5:     if(GameState.getDev_mode()) add_loot(HEALTH,vLoot); break;
            // GUARDAR LOOT
            case ALLEGRO_KEY_PAD_6:     if(GameState.getDev_mode()) save_loot(vLoot); break;
            // AGREGAR ENEMIGOS
            case ALLEGRO_KEY_F5:    if(GameState.getDev_mode())add_enemy(EASY,vEnemies,live_vEnemies); break;
            case ALLEGRO_KEY_F6:    if(GameState.getDev_mode())add_enemy(NORMAL,vEnemies,live_vEnemies); break;
            case ALLEGRO_KEY_F7:    if(GameState.getDev_mode())add_enemy(HARD,vEnemies,live_vEnemies); break;
            case ALLEGRO_KEY_F8:    if(GameState.getDev_mode())save_enemies(vEnemies); break;
            ...
            case ALLEGRO_KEY_F2:    if(GameState.getGame_info()) GameState.changeGame_info(false);
                                ...
                                break;
            case ALLEGRO_KEY_F3:    if(GameState.getDev_mode()) dev_mode_off(); else dev_mode_on();  break;
    ...
    if(GameState.getRedraw()){
        ...
        for(int i=0; i<MAX_ENEMIES; i++){
            ...
            if(vEnemies[i].getLive()){
                ...
                if(collision::circle_circle(vEnemies[i].getX(),vEnemies[i].getY(),10,
                    ...
                    if(rand()%60==0)player.hit_recived(vEnemies[i].getDamage());
                ...
                for(int j=0; j<MAX_BULLETS; j++)
                    if(vBullets[j].getLive())
                        if(collision::circle_circle(vEnemies[i].getX(),vEnemies[i].getY(),vEnemies[i].getRadio(),
                            ...
                            // Enemigo recibe daño. Si muere, el usuario adquiere creditos y puntos
        ...
        if(collision::boundingBox_pixel(Cobra.getX(),Cobra.getY(),Cobra.getX()+128,Cobra.getY()+352,player.getX()-camx,player.getY()-camy)
           and !Cobra.getPlayer_in()){

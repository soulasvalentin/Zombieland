...
struct bitmap{
    ...
    void destroy(){al_destroy_bitmap(bmp); delete location;}
    bitmap &operator=(const bitmap &aux){
        this->bmp = aux.bmp;
        this->w = aux.w;
        this->h = aux.h;
        strcpy(this->location,aux.location);
    }
};
...
enum GAME_STATE     {MAINMENU,OPTIONS,INGAMEMENU,STORE,PRE_GAME,START_PLAYING,PLAYING,HELP,
                     GAMEOVER,RANKING,SAVE_GAME,LOAD_GAME,EXIT};
char *GAME_STATE_t[]={"MAINMENU","OPTIONS","INGAMEMENU","STORE","PRE_GAME","START_PLAYING","PLAYING","HELP",
                      "GAMEOVER","RANKING","SAVE_GAME","LOAD_GAME","EXIT"};
enum DIREC          {ESTE,SUDESTE,SUR,SUDOESTE,OESTE,NOROESTE,NORTE,NORESTE,QUIETO};
...
char *ENEMY_AI_t[] ={"EASY","NORMAL","HARD"};
enum LOOT_TYPE      {AMMO_PISTOL,AMMO_SHOTGUN,AMMO_UZI,AMMO_ASSAULT,HEALTH};
enum MENU_ACTION    {GOUP,GODOWN,BACK,SELECT,QUIT,NONE};
enum COL_TYPE       {WALL,WATER};
enum PLAYER_STATE   {IDLE,WALKING,SHOOTING};
enum BUTTON_STATE   {NONE_,MOUSEOVER,CLICKHOLD,RELEASE};
enum KEYS           {UP,DOWN,LEFT,RIGHT,SPACE,ONE,TAB,RIGHTCLICK};
...
float camx = 0, camy = 0;       // Coordenadas de la cámara
...
void imagen(char *BMP, bool redimensionar, int num = 0){
...
/// Copiar al clipboard (para hacer coliciones)
void copiar(){
